...
# XXD Panel 131｜Doodle Photo Collage
...
Extract only the photograph’s most recognisable **subject, silhouette, structure, pose and narrative relationships** for the lower half, reconstructing them as a **Hand-drawn Doodle Photo Collage**. Do not copy the whole photograph or preserve every object and background. Remove irrelevant details, selecting only a few photographic fragments that best represent its theme and visual memories. Reinterpret them through cropping, scaling, rearrangement and whitespace so that their correspondence with the photograph above is immediately recognisable.
...
The overall effect combines **a white background with extensive open space, small-scale photographic collage, black hand-drawn doodle lines, magazine-like layout and a personal-record quality**. Avoid full replication, excessive background retention, filled canvases, complex scenes, heavy decoration, commercial templates, cheap scrapbooking and excessive flamboyance.
...
- `design-only`: the full canvas contains only Panel 131's designed translation; the photograph remains a non-visible reference.
...
```bash
git clone https://github.com/nevertoday/xxd-panel-131.git
npx skills add https://github.com/nevertoday/xxd-panel-131 --skill xxd-panel-131
```

Restart the agent session after installation, then invoke `$xxd-panel-131`. Add `--global --agent codex --yes` when a user-level Codex installation is wanted.

Common examples:

```text
/xxd-panel-131 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale en-US
/xxd-panel-131 photo.jpg --mode left-right --size 16:9 --text prompt --locale en-US
/xxd-panel-131 photo.jpg --mode design-only --size 9:16 --text none
/xxd-panel-131 ./photos --mode design-only --size auto,3:4 --text prompt --locale ja-JP
```

See [SKILL.md](SKILL.md) for the full runtime contract and the [English](references/xxd-panel-131-prompt.en.md) or [Chinese](references/xxd-panel-131-prompt.zh-CN.md) runtime adapter.
